...
# 1. DDL (AI-Optimized)
# Note: We specify the schema 'rt' explicitly
DDL = """
CREATE TABLE rt.risk_campaign_blacklist (
...
**Critical Rules:**
1. **Schema:** The table is in the `rt` schema (`rt.risk_campaign_blacklist`).
...
EXAMPLES = [
    {
        ...
        "sql": "SELECT user_code, reason, start_date FROM rt.risk_campaign_blacklist WHERE user_code = '10034920';"
    },
    {
        "question": "Check if these users are in the blacklist: 1001, 1002, 1003.",
        "sql": "SELECT user_code, reason FROM rt.risk_campaign_blacklist WHERE user_code IN ('1001', '1002', '1003');"
    },
    {
        "question": "How many of the provided users are blocked?",
        "sql": "SELECT COUNT(*) as blocked_count FROM rt.risk_campaign_blacklist WHERE user_code IN ('ID1', 'ID2', 'ID3');"
    },
    {
        "question": "Show me the reason why user 555 was blocked.",
        "sql": "SELECT user_code, reason, owner FROM rt.risk_campaign_blacklist WHERE user_code = '555';"
    }
]