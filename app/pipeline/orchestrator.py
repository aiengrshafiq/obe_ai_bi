...
class Orchestrator:
    ...
    async def run_pipeline(self, user_msg: str, history_context: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Robust SQL Check: Look for SELECT/WITH and ensure it's not a refusal
            ...
            # Smarter Sanitization (Don't fill NaNs with 0, keeps chart gaps real)
            df = self._sanitize_dataframe(df)

            # --- STEP 5: VISUALIZATION INTELLIGENCE ---
            # Pass intent_result so Viz doesn't have to guess
    ...
    def _build_prompt(self, msg, history, intent):
        ...
        # Calculate dynamic start date for "Last 7 Days" logic
        ...
        1. **INCREMENTAL (_di):** `dws_all_trades_di`, `dws_user_deposit...`, `dwd_login...`.
           - Trend? Use RANGE: `WHERE ds BETWEEN '{start_7d}' AND '{latest_ds}'`
           - Snapshot? `WHERE ds = '{latest_ds}'` (Only shows today's activity!)
        
        2. **SNAPSHOT (_df):** `user_profile_360`, `ads_total_root...`.
           - Current State? Use `WHERE ds = '{latest_ds}'`
        
        3. **TIME:** NEVER use `NOW()`. Use `ds` or timestamp columns anchored to `{latest_ds_iso}`.
    ...
    def _sanitize_dataframe(self, df):
        try:
            # Only remove Infinity. Leave NaN for Plotly to handle (gaps in lines).
